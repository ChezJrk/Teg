...
class TestTuples(TestCase):
    ...
    def test_tuple_basics(self):
        ...
        v2 = TegTuple(*[TegConditional(x, TegConstant(1), TegConstant(2))
                        for i in range(5, 15)])
    ...
    def test_tuple_branch(self):
        ...
        cond = TegConditional(x, v, TegConstant(1))
...
class VariableBranchConditionsTest(TestCase):
    ...
    def setUp(self):
        self.zero, self.one = TegConstant(0), TegConstant(1)

    def test_deriv_heaviside(self):
        x = TegVariable('x')
        t = TegVariable('t', 0.5)
        heaviside = TegConditional(x - t, self.zero, self.one)
        integral = TegIntegral(self.zero, self.one, heaviside, x)
        ...
        neg_heaviside = TegConditional(x - t, self.one, self.zero)
        integral = TegIntegral(self.zero, self.one, neg_heaviside, x)
...
class AffineConditionsTest(TestCase):
    ...
    def test_affine_condition_simple(self):
        x, t = TegVariable('x'), TegVariable('t', 0.25)
        cond = TegConditional(x - TegConstant(2) * t, self.zero, self.one)
        integral = TegIntegral(self.zero, self.one, cond, x)
    ...
    def test_affine_condition_with_constants(self):
        x, t = TegVariable('x'), TegVariable('t', 7/4)
        cond = TegConditional(x + TegConstant(2) * t - TegConstant(4), self.zero, self.one)
        integral = TegIntegral(self.zero, self.one, cond, x)
    ...
    def test_affine_condition_multivariable(self):
        ...
        x, t1, t2 = TegVariable('x'), TegVariable('t1', 3/4), TegVariable('t2', 1/3)
        cond = TegConditional(x - TegConstant(2) * t1 + TegConstant(3) * t2, self.zero, self.one)
        integral = TegIntegral(self.zero, self.one, cond, x)

        deriv_integral = TegReverseDeriv(integral, TegTuple(TegConstant(1)))
        check_nested_lists(self, evaluate(deriv_integral), [1, 1])

        deriv_integral = TegFwdDeriv(integral, [(t1, 1), (t2, 0)])
        self.assertEqual(evaluate(deriv_integral, ignore_cache=True), 1)

        deriv_integral = TegFwdDeriv(integral, [(t1, 0), (t2, 1)])
        self.assertEqual(evaluate(deriv_integral, ignore_cache=True), 1)

    def test_affine_condition_multivariable_multiintegral(self):
        x, y = TegVariable('x'), TegVariable('y')
        t1, t2 = TegVariable('t1', 1/4), TegVariable('t2', 1)
        cond = TegConditional(x - y + TegConstant(2) * t1 + TegConstant(3) * t2 + TegConstant(-3), self.one, self.zero)
        body = TegIntegral(self.zero, self.one, cond, y)
        integral = TegIntegral(self.zero, self.one, body, x)
