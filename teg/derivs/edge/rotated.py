...
def extract_coefficients_from_affine(expr: ITeg, not_ctx: Set[Tuple[str, int]]) -> List[ITeg]:
    if isinstance(expr, Mul):
...
def bounds_of(linear_set: Dict[Tuple[str, int], ITeg],
    ...
    if target_index == 0:
        ...
        for j in range(num_vars):
            placeholder_lb = Placeholder(signature=f'{source_vars[j].uid}_lb')
            placeholder_ub = Placeholder(signature=f'{source_vars[j].uid}_ub')
...
def rotated_delta_contribution(expr: Teg,
    ...
    for discont_bool in extract_moving_discontinuities(expr.body, expr.dvar, not_ctx.copy(), set()):
        try:
            ...
            # Extract rotation.
            raw_affine_set = extract_coefficients_from_affine(discont_bool.left_expr - discont_bool.right_expr, not_ctx)
            ...
            normalization_map = {(normalization_var.name, normalization_var.uid): normalization_expr}
            remapping = partial(TegRemap,
                                map={(s.name, s.uid): (t.name, t.uid) for s, t in zip(source_vars, target_vars)},
                                exprs={**rotated_exprs, **normalization_map},
                                upper_bounds={(var.name, var.uid):
                                              bounds_of(normalized_set, 1 + target_idx, target_vars, source_vars)[1]
                                              for target_idx, var in enumerate(target_vars[1:])},
                                lower_bounds={(var.name, var.uid):
                                              bounds_of(normalized_set, 1 + target_idx, target_vars, source_vars)[0]
                                              for target_idx, var in enumerate(target_vars[1:])},
