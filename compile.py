...
def emit(expr: ITeg, num_samples: int = 50, ignore_cache: bool = False, target = 'C', fn_name = 'teg_program', types = {}):
    assert target in TARGETS.keys(), f'Target language "{target}" is not supported at this time.'
    emitter = TARGETS['C']()

    method = emitter.method(_emit(expr, num_samples, ignore_cache, emitter, type_ctx = types), method_name = fn_name)

    return method

def _emit(expr: ITeg, num_samples: int = 50, ignore_cache: bool = False, emitter = None, type_ctx = None):
    assert emitter is not None, f'Use emit() instead of _emit()'
    ...
    if isinstance(expr, Const):
        ...
        if expr.value is None:
            assert expr.name in type_ctx.keys(), f'"{expr.name}"" does not have a type binding.'
            ctype, size = type_ctx[expr.name]
        else:
            ctype = 'float' if type(expr.value) in [float, int] else None
            size = 1
            assert ctype is not None, f"Default values other than float or integer are not supported"

        code = emitter.variable(expr.name, ctype = ctype, size = size, assigned = False, default = expr.value)
        return code
        ...
        new_bindings = {expr.dvar.name: (lower.out_var.ctype, lower.out_var.size)}
        type_ctx = merge(type_ctx, new_bindings)

        loop_body = _emit(expr.body, num_samples, ignore_cache, emitter, type_ctx)

        ctype, size = new_bindings[expr.dvar.name]
        bind_var = emitter.variable(expr.dvar.name, ctype = ctype, size = size)
        ...
        for var, e in zip(expr.new_vars, expr.new_exprs):
            ...
            assignment_list.append(
                    emitter.assign(
                        emitter.variable(var.name, ctype = var_code.out_var.ctype, size = var_code.out_var.size).out_var,
                        var_code
                    )
                )
            new_bindings[var.name] = (var_code.out_var.ctype, var_code.out_var.size)
        ...
        return emitter.block(assignment_code + expr_code + final_assign_code)
